...
export class ActionExecutorService {
  ...
  constructor(
    ...
    // private readonly roscaService: RoscaService,
  ...
  private async executeSendMoney(intent: Intent, userId: string): Promise<ActionResult> {
    ...
    // In a real implementation, this would call the payments service
    // const result = await this.paymentsService.initiate({...});

    return {
      success: true,
      requiresConfirmation: true,
      message: `Ready to send ${currency?.value || 'USD'} ${amount.value} to ${recipient.value}. Please confirm this transaction.`,
      confirmationData: {
        action: 'send_money',
        amount: amount.value,
  ...
  private async executeCheckBalance(intent: Intent, userId: string): Promise<ActionResult> {
    const accountType = intent.entities.account_type?.value || 'main';

    // Mock data - in real implementation, fetch from WalletsService
    const mockBalances = {
  ...
  private async executeViewTransactions(intent: Intent, userId: string): Promise<ActionResult> {
    const dateRange = intent.entities.date_range?.value || '30d';
    const limit = intent.entities.limit?.value || 10;
  ...
  private async executeInvest(intent: Intent, userId: string): Promise<ActionResult> {
    ...
    return {
      success: true,
      requiresConfirmation: true,
      message: `Ready to invest ${currency?.value || 'USD'} ${amount.value} in ${investment_name.value}. Let me show you the details.`,
      confirmationData: {
        action: 'invest',
        amount: amount.value,
        currency: currency?.value || 'USD',
  ...
  private async executeViewInvestments(intent: Intent, userId: string): Promise<ActionResult> {
    // Mock portfolio data
    const mockPortfolio = {
  ...
  private async executeSearchInvestments(intent: Intent, userId: string): Promise<ActionResult> {
    ...
    return {
      ...
      data: {
        ...
        filters: { category: category?.value, riskLevel: risk_level?.value },
      },
      message: `I found ${filteredResults.length} investment opportunities matching your criteria.`,
    };
  }

  private async executeCreateRosca(intent: Intent, userId: string): Promise<ActionResult> {
    return {
      success: true,
      requiresConfirmation: true,
  ...
  private async executeJoinRosca(intent: Intent, userId: string): Promise<ActionResult> {
    ...
    if (roscaName) {
      const circle = mockCircles.find((c) =>
        c.name.toLowerCase().includes(roscaName.toLowerCase()),
      );
  ...
  private async executeViewRosca(intent: Intent, userId: string): Promise<ActionResult> {
    // Mock user's ROSCA circles
  ...
  private async executeApplyLoan(intent: Intent, userId: string): Promise<ActionResult> {
  ...
  private async executeLendMoney(intent: Intent, userId: string): Promise<ActionResult> {
  ...
  private async executeViewLoans(intent: Intent, userId: string): Promise<ActionResult> {
    // Mock loan data
    const mockLoans = {
      borrowed: [
        {
  ...
  private async executeUpdateProfile(intent: Intent, userId: string): Promise<ActionResult> {
    return {
      success: true,
  ...
  private async executeAddPaymentMethod(intent: Intent, userId: string): Promise<ActionResult> {
    return {
      success: true,
  ...
  private async executeVerifyKYC(intent: Intent, userId: string): Promise<ActionResult> {
    return {
      success: true,
  ...
  private async executeGetHelp(intent: Intent, userId: string): Promise<ActionResult> {
    const topic = intent.entities.topic?.value;
...
What would you like to do?`,
  ...
  private async executeFAQ(intent: Intent, userId: string): Promise<ActionResult> {
    const category = intent.entities.question_category?.value;
